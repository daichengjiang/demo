...
$(function() {
	...
	grid.parameters['entity'] = "placement";
	grid.load();
...
function option(cid,mid,op) {
	...
	if(op == 1){
		title = "添加";
		url = sys.rootPath + '/quotation/add?entity=placement';
	}else if(op == 2){
		title = "编辑";
		url = sys.rootPath + '/quotation/editPlacement?commodityId='+cid+'&marketId='+mid;
	}
	layer.open({
	    type: 2,
	    title: title,
...
function changeStatus(status){
	...
	// 按钮
		...
		$.ajax({
			...
			type : "post",
			data : {"ids" : ids,"status" : status, "entity": "placement"},
