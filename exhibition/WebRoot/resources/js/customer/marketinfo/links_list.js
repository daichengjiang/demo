...
var dtGridOption = {
    ...
    ajaxLoad : true,
    check : false,
    loadURL : sys.rootPath + '/links/list',
...
function option(id,op) {
	...
	if(op == '1'){
		title = "添加";
		url = sys.rootPath + '/links/add';
	}else if(op == '2'){
		title = "编辑";
		url = sys.rootPath + '/links/edit?marketId='+id;
	}
	layer.open({
	    type: 2,
	    title: title,
...
function update(data){
	$.ajax({
		url : sys.rootPath  + "/links/updateProperty",
		type : "post",
		data : data ,
