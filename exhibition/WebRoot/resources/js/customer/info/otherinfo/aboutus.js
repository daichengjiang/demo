...
},
{
	...
	resolution : function(value, record, column, grid, dataNo, columnNo) {
		var str = '';
		str += '<button id="'+record.id+'" class="btn btn-primary btn-sm" style="border-radius:5px 5px;padding:1px 6px;" onclick="updateAboutUs('+record.id+')">修改</button>';
		return str;
	}
}
];
...
var dtGridOption = {
    ...
    ajaxLoad : true,
    check : true,
	loadURL : sys.rootPath + '/otherinfo/aboutuslist',
...
function deleteAboutUs(){
	...
	// 按钮
		...
		$.ajax({
			url : sys.rootPath + '/otherinfo/deleteaboutus',
			type : "post",
			data : {
